...
class NetworkRequestUtil {
  ...
  upload(String url, FormData formData) {
    return handleResponse(dio.post(url, data: formData));
  }
  ...
  refreshTokenFunction() async {
    ...
    final result = await newRequest.post('${Constants.BASE_URL}refreshToken',
        options: Options(headers: {"Authorization": 'Bearer ${refreshToken}'}));
    tokenManager.setToken(result.data["data"]["token"],
  ...
  NetworkRequestUtil() {
    ...
    dio.options.connectTimeout =
        const Duration(seconds: Constants.NEW_WORK_TIME_OUT);
    dio.options.receiveTimeout =
        const Duration(seconds: Constants.NEW_WORK_TIME_OUT);
    dio.options.sendTimeout =
        const Duration(seconds: Constants.NEW_WORK_TIME_OUT);

    dio.interceptors.add(InterceptorsWrapper(onRequest: (context, handler) {
      // 请求拦截器
      ...
      if (context.response?.statusCode == 401) {
        // 处理401问题 换token
        String refreshToken =
            tokenManager.getRefreshToken(); // 获取本地存储的refreshToken
